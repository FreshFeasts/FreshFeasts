...
const OrderHistory = ({ history, setHistory}) => {
  ...
  return (
    <>
    {loading ? <Text>Orders are loading...</Text> :
    <ScrollView bounces={false}>
    <View className="flex-1 justify-center items-center">
      {orders.map((order) => {
        const formattedDate = format(parseISO(order.deliveryDate), 'MM/dd/yyyy');
       return (
        <>
       <AppText className="text-xl" key={order.orderId}>{formattedDate}</AppText>
      {order.meals.map((meal) =><HistoryCard key={meal.name} meal={meal}/>)}
      </>)
    })}
